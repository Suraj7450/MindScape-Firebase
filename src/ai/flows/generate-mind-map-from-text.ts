...
const prompt = ai.definePrompt({
  name: 'generateMindMapFromTextPrompt',
  input: { schema: GenerateMindMapFromTextInputSchema },
  output: { schema: GenerateMindMapFromTextOutputSchema },
  prompt: `You are an expert in analyzing text and creating structured, comprehensive mind maps from it.

  Analyze the following text and generate a detailed, multi-layered mind map based on its content. 
...
const generateMindMapFromTextFlow = ai.defineFlow(
  {
    name: 'generateMindMapFromTextFlow',
    inputSchema: GenerateMindMapFromTextInputSchema,
    outputSchema: GenerateMindMapFromTextOutputSchema,
  },
  async (input) => {
